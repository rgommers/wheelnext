...
### Option 1 — PEP-specified markers, installer-implemented detection

A PEP defines an extended, standardized set of compatibility markers covering the variant axes (CUDA runtime version, ROCm, CPU instruction sets, etc.) and their matching semantics. Wheels declare requirements like `Requires-Variant: cuda_runtime_version >= "12.0"`; each installer implements detection for the standardized markers. The spec is deliberately silent on *how* a tool measures each marker — implementation is each tool's own business; the spec fixes only what the markers mean and how matching behaves.

A hardcoded-without-a-spec variant of this option is not worth separate assessment: installer maintainers will not — and should not — ship compatibility semantics that no standard defines, so any realistic form of this option is PEP-based. Note also that this is *not* Option 4, which requires the marker values to be user-supplied rather than installer-detected.

- **Who maintains CUDA-specific code:** the pip team, the uv team, the Poetry team — each implementing the same PEP-fixed semantics separately. Vendors contribute through the spec process and through PRs to individual tools.
- **How a new accelerator gets supported:** the marker set is extended through global spec approval; then each installer implements and ships support in a release. Two serial gates: the spec process, then N implementation cycles.
- **Trust model:** none — the spec defines only declarative markers and matching semantics, and can state that the how of the implementation is none of its business. The code that performs detection is first-party installer code; no third-party code-execution interface exists at any point.

Option 2 also requires a PEP; the difference is not spec-versus-no-spec but one-time versus recurring. Option 2 standardizes the provider *interface* once, after which new axes ship as provider packages with no further spec changes; Option 1 standardizes each *axis*, so every new hardware dimension repeats the approval-plus-N-implementations cycle. That recurrence is what C1 and SYS-007 measure.
...
| Property            | Option 1 — PEP markers                              | Option 2 — providers                                                      | Option 2a — providers, no guardrails                                | Option 3 — per-package                                              | Option 4 — user-declared                                              |
|---------------------|---------------------------------------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------|---------------------------------------------------------------------|-----------------------------------------------------------------------|
| **C1 Extensibility** | ⛔ Poor — every new dimension needs a spec change plus implementation in every installer | ✅✅ Strong — providers ship independently                                     | ✅✅ As Option 2                                                        | ⚠️ Strong per package, weak across the ecosystem                       | ⚠️ Moderate — new dimensions require spec-level marker namespace extension, but no installer code |
| **C2 Time-to-support** | ⛔ Slowest — global spec approval, then each installer's release cycle | ✅ Weeks — provider publishes when ready                                     | ✅✅ Best of any option — provider publishes and is picked up with no installer or user step | ⚠️ Per package; no shared timeline                                     | ⚠️ Spec change first (months); after that, immediate per user            |
| **C3 Trust surface** | ✅✅ None — the spec is purely declarative; detection is first-party installer code (but see Discussion) | ⚠️ New: provider code runs at resolution. Mitigated by PROV-002, PROV-003 and OV-003   | ⛔ Worst of any option — automatic resolution-time execution of code named by untrusted metadata | ⚠️ Per package; detection runs at install for every variant-shipping package | ✅✅ None — no detection code runs at all; all inputs user-supplied              |
| **C4 Maintenance allocation** | ⛔ Concentrated on installer teams — N parallel implementations of the same semantics; vendors limited to the spec process and per-tool PRs | ✅✅ Distributed to vendors and community; installer stays generic             | ✅✅ As Option 2                                                        | ⛔ Duplicated across packages; each author maintains the same logic    | ⚠️ Distributed to users (and optionally to vendor-shipped helper tools outside the spec) |
...
| **C8 Missing detection** | ⚠️ Tools lag spec extensions — wheels using new markers fall back to the null variant on older installers, so behavior varies by installer version | ✅ Falls back to null variant per RES-004; explainable                       | ✅ Provider auto-installed, so rarely missing; falls back to null variant per RES-004 when it cannot be fetched | ⚠️ Falls back per package; behavior varies                             | ✅ Falls back to null variant if user has not declared values; explainable |
...
**The trust calculation in Option 2 is not new code execution; it is new *timing* of code execution.** Installing any package already runs arbitrary setup code under the existing model. What changes with providers is that the trusted code now runs during *resolution*, not only during install. That is a real shift, but it is a smaller one than "introducing arbitrary code execution into pip," which is sometimes how the concern is framed in passing. The mitigations in PROV-003 (the user can enumerate which providers will run) and OV-003 (the user can disable specific providers) narrow it further. Moreover, resolution-time system inspection is not new at all: installers already run non-trivial detection code during resolution today — manylinux/musllinux compatibility is determined by vendored `packaging` code probing the platform, down to parsing the interpreter's ELF headers to identify the C library and querying its version. A vendored variant provider is substantively the same artefact — installer-shipped detection code running at the same point in the lifecycle — which is why vendoring keeps Option 2 within SEC-007's baseline of "no surprise code execution beyond what is already true today for a default-configured installer." The difference is largely one of perception: libc detection is *ambient* (always on, invisible, never opted into), while provider-based GPU detection is explicit and runs on demand — and explicitness tends to read as more dangerous precisely because it is visible enough to control. The timing, too, is a default rather than an essential property: providers can be run ahead of time, feeding their results into resolution as static data (the same mechanism §Recommendation endorses for cross-target builds), at which point resolution-time code execution is exactly zero. The ambient-versus-explicit perception gap has historically carried real weight in packaging-community discussions, so the providers PEP should make this comparison explicitly rather than assume reviewers will draw it themselves.
...
**Option 1's trust story is genuinely clean; its cost hides in maintenance and time.** The spec can honestly declare that how detection is implemented is none of its business, and the resulting C3 rating — no third-party code-execution interface anywhere — is earned. What the trust column does not show is that the detection logic still has to exist and be maintained by humans: every installer team separately becomes responsible for understanding every accelerator and ABI on every platform, implementing the same PEP-fixed semantics N times. Bugs in CUDA detection get fixed by installer teams, on installer release cadences, regardless of NVIDIA's urgency — and a newly specified marker is only as real as its slowest widely-deployed implementation. That burden lands on C1, C2, and C4, and it is why the clean C3 rating does not settle the choice.
